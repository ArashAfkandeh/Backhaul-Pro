...
func (c *UdpTransport) channelDialer() {
	...
	for {
		...
		case <-c.ctx.Done():
			return
		default:
			// اصلاح فراخوانی TcpDialer
			tunnelTCPConn, err := TcpDialer(c.ctx, c.config.RemoteAddr, c.config.DialTimeOut, c.config.DialTimeOut, true, 3, 0, 0, c.logger)
			if err != nil {
...
func (c *UdpTransport) handleTunnelConn(tunConn *net.UDPConn) {
	...
	for {
		...
		// Parse the remote address and port from the received buffer
		remoteAddrPort := string(buffer[:n])
		host, portStr, err := net.SplitHostPort(remoteAddrPort)
		if err != nil {
			c.logger.Error("failed to parse remote address and port:", err)
			atomic.AddInt32(&c.poolConnections, -1)
			return
		}
		port, err := strconv.Atoi(portStr)
		if err != nil {
			c.logger.Error("failed to convert port to integer:", err)
			atomic.AddInt32(&c.poolConnections, -1)
			return
		}
		remoteAddr := host
