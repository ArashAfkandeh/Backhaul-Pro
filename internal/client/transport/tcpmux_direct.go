...
func (c *TcpMuxTransport) parsePortMappings() {
...
func (c *TcpMuxTransport) startListeners(localAddr string, cfg utils.ListenerConfig) {
...
func (c *TcpMuxTransport) localListener(localAddr string, cfg utils.ListenerConfig) {
...
func (c *TcpMuxTransport) acceptLocalConn(listener net.Listener, cfg utils.ListenerConfig) {
	for {
		select {
		case <-c.ctx.Done():
			return
...
func (c *TcpMuxTransport) handleLoopDirect() {
	for {
		select {
		case <-c.ctx.Done():
			return
...
func (c *TcpMuxTransport) bridgeDirect(localConn LocalTCPConn, session *smux.Session) {
	...
	c.logger.Debugf("connected to direct tcpmux tunnel successfully, remote: %s", localConn.remoteAddr)
