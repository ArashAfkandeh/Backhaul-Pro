...
ports = [
  ...
  "de1.nomac.ir:6701=45.67.139.173:8080",  # معادل in2.infovip.top
...
توجه: سیستم فعلی مسیریابی مبتنی بر Host را با تطبیق دقیق (exact match)
پشتیبانی می‌کند — یعنی مقدار Host/SNI دریافتی با کلیدهای سمت چپِ `ports`
مقایسه می‌شود (case-insensitive). Wildcard (*.example.com) فعلاً به‌صورت
خودکار پشتیبانی نشده؛ در صورتی که بخواهید، می‌توانم گزینه‌ی wildcard/suffix
را اضافه کنم (اولویت: exact → wildcard خاص‌تر → عمومی → fallback).
```
